...
public class LoansServiceClient {
    ...
    private LoansServiceClient(RestTemplate restTemplate, ObjectMapper mapper,
                              @Value("${app.loans-service.host}") String loanServiceHost,
                              @Value("${app.loans-service.port}") String loanServicePort) {
        this.restTemplate = restTemplate;
        this.mapper = mapper;

        LOAN_SERVICE_BASE_URL = "http://" + loanServiceHost + ":" + loanServicePort + "/api/v1/memberRecords";
    }

    public LoanResponseModel getLoan(String memberId, String loanId) {
        try{
    ...
    public LoanResponseModel updateLoan(LoanRequestModel loanRequestModel, String memberId, String loanId) {
        try {
            ...
            HttpEntity<LoanRequestModel> requestEntity = new HttpEntity<>(loanRequestModel);
            ResponseEntity<LoanResponseModel> response = restTemplate.exchange(
                    url, HttpMethod.PUT, requestEntity, LoanResponseModel.class);
